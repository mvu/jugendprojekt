 ...
 * and enables it to control the Adafruit PCA9635 16-Channel 12-bit
...
/**
 * Setup a PCA9635 device with wiringPi.
 *  
 ...
 */
int pca9635Setup(const int i2cAddress)
{
	...
	//int settings = wiringPiI2CReadReg8(fd, PCA9635_MODE1) & 0x7F;
	//int autoInc = settings | 0x20;

	wiringPiI2CWriteReg8(fd, PCA9635_MODE1, 0x00);
	wiringPiI2CWriteReg8(fd, PCA9635_MDOE2, 0x0D); // D
...
void pca9635PWMWrite(int fd, int pin, int pwm)
{
	...
	//wiringPiI2CWriteReg16(fd, reg, pwm & 0x0FFF);
...
//
//	if (value >= 4096)
//		PCA9635FullOn(fd, ipin, 1);
//	else if (value > 0)
//		PCA9635PWMWrite(fd, ipin, 0, value);	// (Deactivates full-on and off by itself)
//	else
//		PCA9635FullOff(fd, ipin, 1);
//}
