...
void init(){
    if(useWiringPi){            // mainconfig
        pcaHL = wiringPiI2CSetup(PCAHL);
        pcaRGB = wiringPiI2CSetup(PCARGB);
        expander = wiringPiI2CSetup(PORTEXPANDER);
        if(Slider_I2c) slider = wiringPiI2CSetup(ARDUINOSLIDER);

        wiringPiI2CWriteReg8(expander, IODIRA, 0x00);
        wiringPiI2CWriteReg8(expander, IODIRB, 0x0f);
    ...
    for (int i = 2; i <= 18; i++){
        wiringPiI2CWriteReg8(pcaHL, i, 0x00);
        wiringPiI2CWriteReg8(pcaRGB, i, 0x00);
